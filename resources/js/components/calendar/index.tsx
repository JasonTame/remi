...
export function Calendar({ currentDate, taskHistory, onTaskClick }: CalendarProps) {
    ...
    const getTasksForDay = (day: Date) => {
        const completedTasks = taskHistory.filter((history) => isSameDay(new Date(history.completed_at), day));
                                        ...
                                        <TooltipProvider key={`completed-${history.id}`}>
                                            <Tooltip>
                                                            ...
                                                            <span
                                                                className={cn(
                                                                    ...
                                                                    history.task.category?.color
                                                                        ? getCategoryColor(history.task.category.color).split(' ')[0]
                                                                        : 'bg-gray-400',
                                                                )}
                                                            />
                                                            {history.task.title}
                                                ...
                                                <TooltipContent>
                                                    ...
                                                    {history.notes && <p className="text-xs text-muted-foreground mt-1">{history.notes}</p>}
                                                </TooltipContent>
                                            </Tooltip>
                                        </TooltipProvider>
                                    ))}
